...
let load_template template_filename =
  ...
  let () =
    let open Lexing in
    lexbuf.lex_curr_p <-
      { lexbuf.lex_curr_p with pos_fname = template_filename }
  in
...
module Json = struct
  ...
  let pp_error ppf (fname, jsonm_loc, error) =
    ...
    in
    Format.fprintf ppf "%a:@ %t" Mustache.pp_loc loc (fun ppf ->
        match error with
...
let load_json json_filename =
  ...
  try Json.json_of_decoder decoder
  with Json.Error (jsonm_loc, error) ->
    Format.eprintf "%a@." Json.pp_error (json_filename, jsonm_loc, error);
    exit 4
...
let manpage =
  Cmdliner.
    ...
    ; `P
        "$(tname) is a command-line tool coming with the $(i,ocaml-mustache) \
         library,\n\
        \      an OCaml implementation of the Mustache template format.\n\
        \      $(tname) takes a data file and a template file as command-line \
         parameters;\n\
        \      it renders the populated template on standard output."
    ; `P
        "Mustache is a simple and popular template format,\n\
        \      with library implementations in many programming languages.\n\
        \      It is named from its {{..}} delimiters."
    ; `I ("Mustache website", "https://mustache.github.io/")
    ; `I
    ...
    ; `S Manpage.s_options
    ; (* The content of this section is filled by Cmdliner; it is used here to
         enforce the placement of the non-standard sections below. *)
      `S "PARTIALS"
    ; `P
        "The $(i,ocaml-mustache) library gives programmatic control over the \
         meaning of partials {{>foo}}.\n\
        \      For the $(tname) tool, partials are interpreted as template \
         file inclusion: '{{>foo}}' includes\n\
        \      the template file 'foo.mustache'."
    ; `P
        "Included files are resolved in a search path, which contains the \
         current working directory\n\
        \      (unless the $(b,--no-working-dir) option is used)\n\
        \      and include directories passed through $(b,-I DIR) options."
    ; `P
        "If a file exists in several directories of the search path, the \
         directory included first\n\
        \      (leftmost $(b,-I) option) has precedence, and the current \
         working directory has precedence\n\
        \      over include directories."
    ; `S "TEMPLATE INHERITANCE / PARTIALS WITH PARAMETERS"
    ; `P
        "$(i,ocaml-mustache) supports a common extension to the original \
         Mustache specification,\n\
        \      called 'template inheritance' or 'parent partials', or here \
         'partials with parameters'.\n\
        \      In addition to usual partials '{{>foo}}', which include a \
         partial template, one can use\n\
        \      the syntax '{{<bar}} {{\\$param1}}...{{/param1}} \
         {{\\$param2}}...{{/param2}} {{/bar}}' to\n\
        \      pass parameters to the included partial template. Inside this \
         included template, parameters\n\
        \      are used as '{{\\$param}}...{{/param}}', with the inner content \
         being used by default\n\
        \      if this parameter was not specified by the caller."
    ; `P
        "This is typically used to define page layouts that are wrapped \
...
<html>
  ...
  <body>
    ...
    ; `S "CONFORMING TO"
    ; `P
        "The $(i,ocaml-mustache) implementation is tested against\n\
        \      the Mustache specification testsuite.\n\
        \      All features are supported, except for lambdas and setting \
         delimiter tags."
    ; `I ("Mustache specification testsuite", "https://github.com/mustache/spec")
    ; `I
        ( "Semi-official specification of PARTIALS WITH PARAMETERS"
...
let run_command =
  ...
  let template_file =
    ...
    Arg.(
      required & pos 1 (some file) None & info [] ~docv:"TEMPLATE.mustache" ~doc)
  in
  let search_path =
    let includes =
    ...
    let no_working_dir =
      let doc =
        "Disable the implicit inclusion of the working directory\n\
        \                 in the search path for partials."
      in
      Arg.(value & flag & info [ "no-working-dir" ] ~doc)
    in
    let search_path includes no_working_dir =
      if no_working_dir then
        includes
      else
        Filename.current_dir_name :: includes
    in
