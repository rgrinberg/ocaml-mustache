Simple test:
  $ mustache-ocaml data.json foo.mustache
...
Include in child or parent directory:
  ...
  $ mustache-ocaml from_parent.json from_parent.mustache
  Test from parent

  $ mkdir subdir/child
  $ echo "{{> ../test }}" > subdir/child/from_child.mustache
  $ echo '{ "src": "child" }' > subdir/child/from_child.json
  $ (cd subdir/child; mustache-ocaml from_child.json from_child.mustache)
  Test from child
...
This fails:
  $ (cd subdir; mustache-ocaml ../data.json ../foo.mustache)
  File "../foo.mustache", line 2, characters 23-31:
  the partial 'bar' is missing.
  [2]

This works with the "-I .." option:
  $ (cd subdir; mustache-ocaml -I .. ../data.json ../foo.mustache)
  Inside the include is "Foo Bar !"
...
over the include directories added after:
  $ (cd precedence; mustache-ocaml -I first -I last data.json template.mustache)
  First
...
... unless --no-working-dir is used:
  $ (cd precedence; mustache-ocaml --no-working-dir -I first -I last data.json template.mustache)
  First
