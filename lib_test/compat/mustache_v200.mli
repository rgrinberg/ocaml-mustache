...
(** [to_formatter fmt template] print a template as raw mustache to the
    formatter [fmt]. *)
...
(** [render_fmt fmt template json] render [template], filling it with data from
    [json], printing it to formatter [fmt]. *)
val render_fmt : Format.formatter -> t -> Ezjsonm.t -> unit

(** [render template json] use [render_fmt] to render [template] with data from
    [json] and returns the resulting string. *)
